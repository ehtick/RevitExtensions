...
public static class ParameterUtilsExtensions
{
    ...
    extension(ForgeTypeId typeId)
    {
        ...
        ///    To retrieve them, the user must be signed in.
        /// </remarks>
        /// <param name="region">Region of the account where the parameter is defined.</param>
        /// <returns>Settings associated with a parameter.</returns>
        /// <exception cref="T:Autodesk.Revit.Exceptions.AccessDeniedException">
        ///    Thrown when the user is not authorized to access the requested information.
        /// </exception>
        ...
        /// </exception>
        [Pure]
        public ParameterDownloadOptions DownloadParameterOptions(string region)
        {
            return ParameterUtils.DownloadParameterOptions(typeId, region);
        }
        ...
        /// </exception>
        [Pure]
        public string DownloadCompanyName(Document document, string region)
        {
            return ParameterUtils.DownloadCompanyName(document, typeId, region);
        }
        ...
        /// </exception>
        [Pure]
        public SharedParameterElement DownloadParameter(Document document, ParameterDownloadOptions options, string region)
        {
            return ParameterUtils.DownloadParameter(document, options, typeId, region);
        }
#endif
    }
